...
struct Interaction
    speciesA::AbstractSpeciesCellularAutomata
    suitabilitySpeciesA::Matrix{Float64}
    speciesB::AbstractSpeciesCellularAutomata
    suitabilitySpeciesB::Matrix{Float64}
    strength::Float64
...
function coloniseNeighbourWeight(ca::SpeciesCellularAutomataNeighbourEffect)
    ...
    for i in idxShuffle
        cellIndex = dCells[i]
        paNeighbourhood = getNeighbourhood(ca.neighbourhoodParams,ca.pa,cellIndex,shp,ca.neighbourhoodParams)
        suitNeighbourhood = getNeighbourhood(ca.neighbourhoodParams,ca.suitabilityActive,cellIndex,shp,ca.neighbourhoodParams)
...
function extinctionNeighbourWeight(ca::SpeciesCellularAutomataNeighbourEffect)
    ...
    for i in idxShuffle
        ...
        if ca.pa[i] === 1.0
            ...
            paNeighbourhood = getNeighbourhood(ca.pa,cellIndex,shp,ca.neighbourhoodParams)
            suitNeighbourhood = getNeighbourhood(ca.suitabilityActive,cellIndex,shp,ca.neighbourhoodParams)
            neighbourWeight = neighbourHoodWeight(paNeighbourhood,suitNeighbourhood,ca.weightMatrix)
...
"""
function interact(interaction::Interaction)
    interaction.speciesA.suitabilityActive = interaction.speciesA.suitabilityInitial .* (1 .- (effectSpeciesBonA .* interaction.speciesB.pa))
    interaction.speciesB.suitabilityActive = interaction.speciesB.suitabilityInitial .* (1 .- (effectSpeciesAonB .* interaction.speciesA.pa))
end
